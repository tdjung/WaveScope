...
def changes_to_ticks(changes: Iterator[Tuple],
    ...
    The period is FIXED for the whole trace.  If the clock frequency
    changes mid-trace (CMU/DVFS), time deltas fall off the period grid;
    that is detected (one modulo per sample) and reported with explicit
    guidance: dump the core clock and use --clock, which counts edges
    and is exact under any frequency schedule.  (An adaptive clockless
    re-lock was tried in v0.11.0 and reverted -- see NOTES.md.)
    """
    import itertools
    import math
            ...
            if prev_t is not None:
                ...
                if d > 0 and d % p:
                    ...
                    if first_t is None:
                        first_t = t
                        print(
                            f"[wavescope] WARNING: PC change at t={t} is "
                            f"off the {p}-unit clock grid. The clock "
                            f"frequency most likely changes mid-trace "
                            f"(CMU/DVFS) -- clockless cycle counts are NOT "
                            f"reliable past this point. Dump the core "
                            f"clock signal and re-run with --clock "
                            f"<signal>: cycles then count actual edges "
                            f"and are exact under any frequency schedule.",
                            file=sys.stderr)
            prev_t = t
            yield ((t - t0 + p // 2) // p,) + tuple(ch[1:])
        if offgrid:
            print(f"[wavescope] WARNING: {offgrid} off-grid PC change(s) "
                  f"total, first at t={first_t}. Cycle results are "
                  f"unreliable -- re-run with the clock dumped and "
